function [Vout, CA, stats] = ccep_CARLA_modified(tt, V, srate, badChs, optsIn)
%
...
%
%    functions/ccep_CARLA_modified.m
...
%   This function was adapted from CARLA on 2023/06/14 by Harvey Huang and Dora Hermes
%   
    ...
    %% subtract virtual channel for ranking purposes

    virtual_channels = stats.order(round(nChs/2)); % pick channel with middle order variance

    VgoodChs2 = VgoodChs - VgoodChs(virtual_channels,:,:);


%% Rank channels again by increasing variance or covariance
    ...
    stats.order(stats.order==virtual_channels) = [];
    stats.order = [stats.order; virtual_channels];
